---
title: Instance Resize
hide_table_of_contents: true
custom_edit_url: null
hide_sidebar: true
sidebar_position: 4
---

import FigmaEmbedWithLoader from '@site/src/components/FigmaEmbedWithLoader';

Resize your virtual machine as needed.

<div style={{ width: "100%", height: "auto", margin: 0, padding: 0, overflow: "hidden" }}>
  <FigmaEmbedWithLoader  className="figma-wrapper"
    url="https://embed.figma.com/proto/zv3VuZ86NQth7RxVMUg8kg/Instance-Resize?node-id=1-56&scaling=scale-down-width&content-scaling=fixed&page-id=0%3A1&embed-host=share"
    thumbnail="/img/template-thumbnail.jpg" 
  />
</div>